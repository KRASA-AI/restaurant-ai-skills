...
version: 1.4
last_eval_score: 9.30  # 2026 currency: ArchIQ/Google edge-cloud pattern, English+Spanish default, ~90% capture / 93-95% accuracy benchmarks, Taco Bell car-by-car dynamic menu board; industry_fit 9->10
---
...
3. **Voice AI platform** — Vendor (Presto, ConverseNow, Hi Auto, Incept AI, SoundHound, Amazon Nova Sonic, a Google-Cloud OEM stack à la McDonald's ArchIQ, or another OEM), contracted language coverage (note that leading 2026 stacks ship live English + Spanish out of the box — McDonald's ArchIQ and Taco Bell's Yum! voice AI both do), accent models, integration pattern (POS-native, KDS sidecar, Deliverect-mediated unified ordering, or an on-prem edge-compute pattern — McDonald's is installing Google Edge Cloud blades in US restaurants to run inference locally rather than round-tripping to the cloud), the reported production capture benchmark to hold the wave against (mature 2026 pilots report ~90% of orders completed without human escalation and 93–95% accuracy on simple-to-moderate orders — use these as external reference points, not as your own week-1 target), and hardware refresh scope (speaker posts, OCS / advance order confirmation screen, digital menu board, edge-compute blade)
...
5. **Advance order confirmation screen copy and layout** — Specify the OCS spec: font size, maximum line length, item stacking order, modifier display, total position, upsell display (yes / no / only-if-accepted), error state ("Press for cashier"), and the exact phrases the screen displays when the AI is still listening ("Got it — anything else?") vs. finished ("Pull forward when you're ready"). Catching errors before the order is fired is the primary accuracy-lift mechanism of a modern AI drive-thru. If the wave includes an **AI-driven dynamic digital menu board** — the 2026 pattern Taco Bell is piloting, where the board changes layout, content, and visuals car-by-car (weather, daypart, current wait, and the in-progress order drive what is shown) — treat it as a distinct surface from the OCS and bound it explicitly: the dynamic board may re-rank promos and swap hero imagery, but it must not alter a price the voice agent already quoted, must honor the same daypart/LTO/availability flags and margin-anchored upsell logic as step 4, and must never show an 86'd or store-unavailable item. Name who owns the board's content rules and confirm the board and the OCS reconcile to the same canonical menu so the guest never sees two different prices between the board and the confirmation screen.

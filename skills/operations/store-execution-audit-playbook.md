...
9. **Integrations with labor, inventory, and guest-complaint data** — Prescribe the three integrations that turn the photo audit from surveillance into diagnosis. Labor: if a store's line-station photo fail rate spikes on Saturdays between 6-9 PM, cross-reference the Staff Schedule Optimizer output — is the line understaffed at that daypart? Inventory: if the 86-board photo keeps failing (items marked available that are actually 86'd), cross-reference the Demand Forecast Briefing and the Food Waste Reduction Planner — is the store receiving the wrong order quantity? Guest complaint: if a single store has three restroom photo fails in a week, pull the guest-complaint data for that store — is the mystery-shopper score dropping too? Then add a fourth, newer integration where the brand runs an autonomous POS-native ops agent (the 2026 manager-copilot wave — PAR's "Samantha," Square's manager agents, and similar — which surface late-night-sales, reconciliation, and labor exceptions on their own; e.g., a Taco Bell franchisee's agent flagging stores that should stay open later, a 40-unit chain cutting ~12 manager overtime hours/week): treat the agent's autonomously-surfaced exceptions as a *signal source into the same anomaly loop*, not a parallel system — route them through this playbook's auto-follow-up task workflow (step 5) and trend-anomaly scan (step 6) so an agent-flagged "store closing too early vs. demand" or "labor variance" lands as a dated, owned task next to the photo-driven fails, and reconcile the two so the above-store team sees one ranked queue rather than two competing dashboards (and so the agent's recommendations still pass a human owner before they change a schedule or hours). The integration design is how the program graduates from "found the problem" to "solved the problem."
...
> **Autonomous ops agent (if running):** the POS-native agent flags Midtown closing at 9pm while late-night demand holds → the exception routes into this playbook's task queue as a dated, owned item (Area-GM review) → cross-ref Demand Forecast Briefing + Staff Schedule Optimizer before any hours change; the agent recommends, the human owns the call. One ranked queue, not two dashboards.
